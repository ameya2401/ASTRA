...
- **Exact Efficiency**: While KernelSHAP requires thousands of random model perturbations, TreeSHAP evaluates tree paths in polynomial time $O(T L D^2)$, allowing feature attribution calculation in under 15 milliseconds per PR.
- **Local Additivity**: For any PR feature vector $x$:
...
1. **Calculate Attributions**: The explainer evaluates the 28 features against the trained ensemble.
2. **Sort by Contribution**:
   - **Risk Amplifiers ($\phi_i > 0$)**: Features that pushed the probability toward defective. Sorted descending to isolate the Top 3 positive contributors.
   - **Mitigating Factors ($\phi_i < 0$)**: Features that pulled the probability toward safe (such as high test coverage or clean CI). Sorted ascending.
3. **Database Storage**: The top 3 positive feature names and their numerical attribution values are persisted in [database/models.py:L263-L270](file:///e:/study/ASTRA/ast-triage-core/database/models.py#L263-L270) (`shap_feature_1..3`, `shap_value_1..3`).

---

## 3. Human-Readable Translation

The reporter translates numerical SHAP outputs into clear developer prose:
